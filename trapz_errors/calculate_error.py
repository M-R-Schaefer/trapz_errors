...
from trapz_errors.helpers import (calc_y_intersection_pt, parse_user_data,
                                  round_sigfigs, rss,
                                  second_derivative_with_uncertainty)
...
def interval_errors(xs, ys, es, forward=True):
    ...
    for i in range(len(xs) - 3):
        gap_xs.append((xs[i + 1] + xs[i + 2]) / 2.0)
...
def trapz_interval_error(pts, dx):
    ...
    return (dx**3) / 12.0 * np.array(second_der)
...
def trapz_integrate_with_uncertainty(xs, ys, es, be_conservative=True):
    ...
    gap_xs, gap_ys, gap_errors = sorted(
        [forward_results, backwards_results], key=lambda x: np.abs(np.sum(x[-1]))
    )[-1]
    point_uncertainty_error = rss(list(integration_point_errors))
    max_interval_error = np.max(
        np.abs(np.concatenate((forward_results[-1], backwards_results[-1])))
    )
...
def config_argparse():
    ...
    argparser.add_argument(
        "-v", "--verbose", action="store_true", help="Output error breakdown."
    )
    argparser.add_argument(
        "-c",
        "--conservative",
...
def process_plot_argument(args):
    ...
    figure_name = (
        "{0}.png".format(figure_name)
        if (figure_name and "." not in figure_name)
        else figure_name
    )
    return figure_name
...
def run(xs, ys, es, figure_name, sigfigs, verbose, be_conservative):
    ...
    result_string = "{0:g} +/- {1:g}".format(
        round_sigfigs(integral, sigfigs + 2), round_sf(total_error)
    )
    if verbose:
        value_error = round_sf(rss(integration_point_errors))
        print("Error from y-value uncertainty: +/- {0:g}".format(value_error))
        print(
            "Estimated total truncation error: {0:g}".format(
                round_sf(np.abs(np.sum(gap_errors)))
            )
        )
        if be_conservative:
            print(
                "Maximum interval error: {0:g}".format(
