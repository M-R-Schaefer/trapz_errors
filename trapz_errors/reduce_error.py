...
from trapz_errors.calculate_error import (config_argparse, parse_user_data,
                                          plot_error_analysis,
                                          process_plot_argument,
...
def reduce_error_on_residual_error(
    error_pts, residule_error, convergence_rate_scaling, be_conservative
):
    ...
    for pt in sorted_error_pts:
        ...
        if be_conservative and pt == largest_gap_error_pt:
            ...
            residule_error -= (1.0 / convergence_rate_scaling) * 0.75 * abs(pt[0])
...
def get_updates(
    ...
    largest_error_pts = reduce_error_on_residual_error(
        combined_pts_errors, residule_error, convergence_rate_scaling, be_conservative
    )
...
def parse_args():
    ...
    argparser.add_argument(
        ...
        "--convergence_rate_scaling",
        type=float,
        default=CONVERGENCE_RATE_SCALING,
...
def run(
    ...
    if verbose:
        ...
        truncation_error = round_sf(
            np.sum(gap_errors) if be_conservative else np.sum(gap_errors)
        )
        print("Estimated truncation error: {0:g}".format(truncation_error))
        if be_conservative:
            print(
                "Additional error component: {0:g}".format(
                    round_sf(conservative_error_adjustment)
                )
            )
        ...
        print(
            "\n".join(
                [
                    "{1:<6.4f} -> {0:>7.4f}".format(*map(round_sf, d))
                    for d in sorted(zip(gap_errors, gap_xs), key=lambda x: abs(x[0]))[
                        ::-1
                    ]
                ]
            )
        ...
        print(
            "\n".join(
                [
                    "{1:<6.4f} -> +/- {0:6.4f}".format(*map(round_sf, d))
                    for d in sorted(zip(integration_point_errors, xs))[::-1]
                ]
            )
        )
    ...
    if total_error > target_error:
        new_pts, update_xs = get_updates(
            ...
            target_error,
            convergence_rate_scaling,
            be_conservative=be_conservative,
        )
        if new_pts:
    ...
    if figure_name:
        plot_error_analysis(
            ...
            gap_errors,
            figure_name,
            title="Integral: {0}".format(result_string),
        )
